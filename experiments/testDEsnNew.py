...
if __name__ == "__main__":
    ...
    for n in range(num):
        ...
        esn = DEsnModel(input_size=2, output_size=1, num_reservoirs=num_reservoirs,
                        reservoir_sizes=[200]*num_reservoirs,
                        echo_params=np.linspace(0.85, 0.5, num_reservoirs).tolist(),
                        spectral_scales=[1.0]*num_reservoirs,
                        input_weight_scales=[0.2]*num_reservoirs,
                        sparsities=[0.01]*num_reservoirs,
                        res_initialise_strategies=['binary']*num_reservoirs,
                        regulariser=1e-5)
        ...
        esn.train(X_train, y_train, data_repeats=reps, warmup_timesteps=warm_up_length)
